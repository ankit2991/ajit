...
class SatelliteViewActivity : BaseActivity(),
    ...
    private fun init() {
        ...
        binding.btn3d2d.visibility = View.VISIBLE

        binding.toolbar.title.text = getString(R.string.satellite_view)
    ...
    private fun setActions() {
        ...
        binding.btn3d2d.setOnClickListener {
            if (binding.btn3d2d.text == "3D") {
                binding.btn3d2d.text = "2D"
                googleMap?.cameraPosition?.let {
                    val cameraPosition =
                        CameraPosition.Builder().target(it.target).tilt(60F).zoom(15F).build()
                    googleMap?.animateCamera(CameraUpdateFactory.newCameraPosition(cameraPosition))
                }

            } else {
                binding.btn3d2d.text = "3D"
                googleMap?.cameraPosition?.let {
                    val cameraPosition =
                        CameraPosition.Builder().target(it.target).tilt(0F).zoom(15F).build()
                    googleMap?.animateCamera(CameraUpdateFactory.newCameraPosition(cameraPosition))
                }
            }
    ...
    private fun getAddressAndShowList(query: String?) {
        ...
        GlobalScope.async {
            var list =
                ApiClientHelper.getLatLngFromAddress(this@SatelliteViewActivity, query.toString())
                ...
                if (list == null) {
                    ...
                    if (suggestedPlacesAdapter == null) {
                        ...
                        binding.rvSuggestion.layoutManager =
                            LinearLayoutManager(this@SatelliteViewActivity)
