...
class BillingService(
    ...
    override fun onPurchasesUpdated(billingResult: BillingResult, purchases: List<Purchase>?) {
        ...
        when (responseCode) {
            ...
            BillingClient.BillingResponseCode.SERVICE_DISCONNECTED -> {
                ...
                log("onPurchasesUpdated: service disconnected")
            }

            BillingClient.BillingResponseCode.ERROR -> {
                QuimeraInit.billingError(context)
                log("onPurchasesUpdated: billing error")
            }
            ...
            BillingClient.BillingResponseCode.BILLING_UNAVAILABLE -> {
                ...
                log("onPurchasesUpdated: billing unavailable")
            }

            BillingClient.BillingResponseCode.ITEM_UNAVAILABLE -> {
                QuimeraInit.itemUnavailable(context)
                log("onPurchasesUpdated: item unavailable")
            }

            BillingClient.BillingResponseCode.ITEM_NOT_OWNED -> {
                QuimeraInit.itemNotOwned(context)
                log("onPurchasesUpdated: item not owned")
            }
        }
    }
    ...
    override fun onBillingServiceDisconnected() {
        ...
        if (billingClient != null && !billingClient.isReady) {
            ...
            billingClient.startConnection(this) //called this when app is in foreground
        }
    }
