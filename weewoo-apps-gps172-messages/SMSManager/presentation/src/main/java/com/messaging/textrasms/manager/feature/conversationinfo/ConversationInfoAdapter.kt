...
import com.messaging.textrasms.manager.feature.conversationinfo.ConversationInfoItem.*
...
class ConversationInfoAdapter @Inject constructor(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): QkViewHolder {
        ...
        return when (viewType) {
            0 -> QkViewHolder(
                inflater.inflate(
                    R.layout.conversation_recipient_list_item,
                    parent,
                    false
                )
            ).apply {
                itemView.setOnClickListener {
                    ...
                    ConversationInfoActivity.isRefreshList = true
                }
            ...
            1 -> QkViewHolder(
                inflater.inflate(
                    R.layout.conversation_info_settings,
                    parent,
                    false
                )
            ).apply {
            ...
            2 -> QkViewHolder(
                inflater.inflate(
                    R.layout.conversation_media_list_item,
                    parent,
                    false
                )
            ).apply {
    ...
    override fun onBindViewHolder(holder: QkViewHolder, position: Int) {
        when (val item = getItem(position)) {
            is ConversationInfoRecipient -> {
                ...
                val isPossibleNumber = PhoneNumberUtils(context).isPossibleNumber(recipient.address)
                holder.add.setVisible(recipient.contact == null && isPossibleNumber)
            ...
            is ConversationInfoSettings -> {
                ...
                holder.archive.title = context.getString(
                    when (item.archived) {
                        true -> R.string.info_unarchive
                        false -> R.string.info_archive
                    }
                )
                holder.block.isVisible = item.recipients.size <= 1
                holder.block.title = context.getString(
                    when (item.blocked) {
