...
import com.messaging.textrasms.manager.feature.adapters.conversations.ConversationsAdapterpersonal.Companion.isMultiSelectall
...
abstract class QkRealmAdapterpersonal<T : RealmModel> :
    ...
    override fun getItem(index: Int): T? {
        ...
        //logDebug("conversationlist" + data!!.get(index))
    ...
    fun selectionAllother(conversations: List<Conversation>) {
        try {
            for (conversion in conversations) {
                if (!isSelected(conversion.id)) {
                    toggleSelection(conversion.id)
