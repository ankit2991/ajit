...
class OtherModel @Inject constructor(
...
) : QkViewModel<OtherMainView, MainState>(
    MainState(
        page = Inbox(
            data = conversationRepo.getConversationsknown(
                false,
                false
            )
        )
    ...
    override fun bindView(view: OtherMainView) {
        super.bindView(view)
        ...
        view.optionsItemIntent
            ...
            .withLatestFrom(state) { _, state ->
                android.os.Handler().postDelayed(Runnable {
                    val conversations = conversationRepo.getConversationsknown(false, false)
        ...
        try {
            when {
                category.equals(0) -> newState {
                    copy(
                        page = Inbox(
                            data = conversationRepo.sortingOfDatasortingOfDataDateknown(
                                false,
                                false,
                                false,
                                field,
                ...
                category.equals(1) -> newState {
                    copy(
                        page = Inbox(
                            data = conversationRepo.sortingOfDatasortingOfDataMonthknown(
                                false,
                                false,
                                false,
                                field,
                ...
                category.equals(2) -> newState {
                    copy(
                        page = Inbox(
                            data = conversationRepo.sortingOfDatasortingOfDataYearknown(
                                false,
                                false,
                                false,
                                field,
                ...
                category.equals(3) -> newState {
                    copy(
                        page = Inbox(
                            data = conversationRepo.getConversationsother(
                                false,
                                false
                            )
                        )
