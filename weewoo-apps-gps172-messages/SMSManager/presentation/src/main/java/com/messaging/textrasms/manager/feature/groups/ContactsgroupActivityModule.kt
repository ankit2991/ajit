...
@Module
class ContactsgroupActivityModule {

    @Provides
    fun provideIsSharing(activity: ContactsgroupActivity): Boolean {
        return activity.intent.extras?.getBoolean(ContactsgroupActivity.SharingKeygroup, false)
            ?: false
    }

    @Provides
    fun provideChips(activity: ContactsgroupActivity): HashMap<String, String?> {
        return activity.intent.extras?.getSerializable(ContactsgroupActivity.ChipsKeygroup)
    ...
    @Provides
    @Named("query")
    fun provideQuery(activity: ContactsgroupActivity): String =
        activity.intent.extras?.getString("query")
            ?: ""

    @Provides
    @Named("threadId")
    fun provideThreadId(activity: ContactsgroupActivity): Long =
        activity.intent.extras?.getLong("threadId")
            ?: 0L

    @Provides
    @Named("addresses")
    fun provideAddresses(activity: ContactsgroupActivity): List<String> {
    ...
    @Provides
    @Named("text")
    fun provideSharedText(activity: ContactsgroupActivity): String {
    ...
    @Provides
    @Named("attachments")
    fun provideSharedAttachments(activity: ContactsgroupActivity): Attachments {
    ...
    @Provides
    @IntoMap
    @ViewModelKey(ContactsGroupViewModel::class)
    fun provideComposeViewModel(viewModel: ContactsGroupViewModel): ViewModel = viewModel
